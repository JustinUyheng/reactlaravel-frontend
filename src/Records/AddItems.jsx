...
const AddItems = () => {
	...
	const [itemsList, setItemsList] = useState(() => {
		...
		const defaultItems = [
			{
				id: 1,
				image: assets.food_1 || assets.placeholder,
				name: "Beef patty burger",
				category: "Meals",
				price: "25",
				description: "A classic beef patty burger.",
			},
			{
				id: 2,
				image: assets.food_2 || assets.placeholder,
	...
	const handleStatusChange = (orderIdToUpdate, newStatus) => {
		...
		try {
			...
			const currentOrderHistory = currentOrderHistoryRaw
				? JSON.parse(currentOrderHistoryRaw)
				: [];
			if (Array.isArray(currentOrderHistory)) {
				const orderFromStateThatChanged = orders.find(
					(o) => o.id === orderIdToUpdate
				);
				const updatedHistory = currentOrderHistory.map((histOrder) => {
					let isMatch = false;
					if (orderFromStateThatChanged) {
						if (
							orderFromStateThatChanged.uniqueOrderId &&
							histOrder.uniqueOrderId ===
								orderFromStateThatChanged.uniqueOrderId
						)
							isMatch = true;
						else if (
							orderFromStateThatChanged.orderId &&
							histOrder.orderId === orderFromStateThatChanged.orderId
						)
							isMatch = true;
						else if (
							!orderFromStateThatChanged.uniqueOrderId &&
							!orderFromStateThatChanged.orderId &&
							histOrder.timestamp === orderFromStateThatChanged.timestamp
						)
							isMatch = true;
					}
					if (isMatch)
						return { ...histOrder, status: newStatus, fee: histOrder.fee };
					return histOrder;
				});
	...
	return (
		<div className="additems-page-container">
				...
				<main className="additems-main-content">
					...
					{activeTab === "addItems" && (
						<form className="additems-form" onSubmit={handleAddClick}>
								...
								<div className="additems-form-section additems-category-section">
									...
									<div className="additems-custom-select">
										<div
											...
											className="additems-select-selected"
											onClick={() =>
										...
										>
											{productCategory}
											<span
												className={`additems-select-arrow ${
										...
										{isCategoryDropdownOpen && (
											<div className="additems-select-options" role="listbox">
												{categories.map((category) => (
													<div
														key={category}
														className="additems-select-option"
														onClick={() => {
							...
							<button type="submit" className="additems-add-button">
								ADD
							</button>
						</form>
					)}
			...
			{showSuccessMessage && (
				<div className="additems-success-notification">
					{" "}
					<CheckIcon /> Food Added Successfully{" "}
				</div>
			)}
		</div>
	);
