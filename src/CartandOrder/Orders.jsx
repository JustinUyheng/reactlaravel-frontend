...
const Orders = () => {
	...
	const removeTransaction = (transactionIdToRemove) => {
		if (
			...
			const updatedHistory = allTransactions.filter(
	...
	const renderTransactionItem = (transaction) => {
		...
		const names = transaction.items
			...
			.join(", ");
		const totalItemsCount = transaction.items.reduce(
		...
		const iconSrc =
			transaction.type === "reservation" ? assets.reserve : assets.box;
		const iconAlt =
			transaction.type === "reservation" ? "Reservation Icon" : "Order Icon";
		// Placeholder if actual image fails to load or isn't defined in assets
		const placeholderIconContent =
			transaction.type === "reservation" ? "🗓️" : "📦";
				...
				<div className="order-details">
					...
					{transaction.userInfo && (
						<p style={{ fontSize: "0.9em", color: "#555" }}>
							Customer: {transaction.userInfo.fullName} (
							{transaction.userInfo.email}, {transaction.userInfo.contactNumber}
							)
						</p>
					)}
					{transaction.pickupInfo && (
						<p style={{ fontSize: "0.9em", color: "#555" }}>
							Pickup: {transaction.pickupInfo.pickupDate} at{" "}
					...
					{transaction.paymentMethod === "gcash" &&
						transaction.paymentDetails && (
							<p style={{ fontSize: "0.9em", color: "#555" }}>
								Gcash Acc: {transaction.paymentDetails.accountName}, No:{" "}
