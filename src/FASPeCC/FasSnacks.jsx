...
const FasSnacks = () => {
	...
	const handleAdd = (item, type) => {
		...
		const totalItems = updatedCart[type].reduce(
			(sum, item) => sum + item.quantity,
			0
		);
		toast.success(
			`Successfully added to ${
				type === "order" ? "Order" : "Reserve"
			}! You have ${totalItems} item(s) in your ${type}.`
		);
	};
					...
					<div className="snack-card" key={index}>
						...
						<div className="snack-info">
							...
							<div className="button-row">
								<button onClick={() => handleAdd(item, "reserve")}>
									Reserve
								</button>
